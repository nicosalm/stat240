...
> The predicted value $\hat{y}$ is always equal to $\bar{y}$ when $x = \bar{x}$, regardless of the values of other summary statistics. This is because the least squares regression line always passes through the point $(\bar{x}, \bar{y})$.
...
> The correct answer is (c). For the same reason as in the previous problem.
...
dugong <- read_csv("../../data/dugong.csv") %>% 
  ...
  rename(length = Length, age = Age)

# scatter plot
ggplot(dugong, aes(x = length, y = age)) +
  geom_point() +
  labs(x = "Length (m)", y = "Age (years)") +
  theme_bw() +
  ggtitle("Dugong Length vs. Age") +
...
mean = mean(dugong$age)
std = sd(dugong$age)
cor = cor(dugong$age, dugong$length)
m = cov(dugong$age, dugong$length) / var(dugong$length)
xint = mean(dugong$age) - cov(dugong$age, dugong$length) / var(dugong$length) * mean(dugong$length)
...
```{r}
# use lm() and coef() to calculate slope and intercept
model <- lm(age ~ length, data = dugong)
coef(model) # hey, look at that, same values!
```
...
```{r}
# add columns with predicted values and residuals
dugong <- dugong %>% 
  mutate(pred = m * length + xint,
         res = age - pred)

# plot residuals vs. length
ggplot(dugong, aes(x = length, y = res)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Length (m)", y = "Residuals") +
  theme_bw() +
  ggtitle("Residuals vs. Length") +
  # resize y-axis to better see residuals
  # add a line
  geom_smooth(method = "lm", se = FALSE) +
  geom_segment(aes(xend = length, yend = pred), color = "red", alpha = 0.5)
```
...
>  We see that the residuals are centered around 0, and that there is no obvious pattern in the residuals. This supports the use of a linear model. However, there are a few outliers that might be problematic for the model.
...
ggplot(boston, aes(x = Age, y = Time)) +
  ...
  labs(x = "Age (years)", y = "Time (minutes)") +
  theme_bw() +
  ggtitle("Boston Marathon 2010: Female Runners") +
  # add least squares line
  geom_smooth(method = "lm", se = FALSE) +
  # add a smooth curve
...
```{r}
# create a column with predicted values and residuals
boston <- boston %>% 
  mutate(pred = m * Age + xint,
         res = Time - pred)

# plot residuals vs. age
ggplot(boston, aes(x = Age, y = res)) +
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
> We see several peaks which suggest that there is strong non-linearity in the data. The residuals are not centered around 0, and there is a clear pattern in the residuals.

- Is there evidence that the standard deviation of the residuals varies substantially with changes in age?

> The first plot against age showed a somewhat consistent spread of residuals across the age range, which would suggest homoscedasticity. But there was a slight fan-out in the residuals for higher ages, which could indicate increasing variance among older runners.


- Is there evidence that the error distribution for individual residuals is not symmetric?

> Yes, the density plot of residuals shows a clear skew to the left. This suggests that the error distribution is not symmetric.
