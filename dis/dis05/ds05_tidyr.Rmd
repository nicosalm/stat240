...
```{r}
# Define the counting function because I don't want to do this four times 
# and I want to learn how these work!
count_observations = function(data, var_name) {
  data %>%
    group_by({{ var_name }}) %>%
    summarise(n = n()) %>%
    arrange(-n)
...
# use it to count each var
variables_to_check = c("date", "block", "position", "lure")

for (var in variables_to_check) { # for loop in R >:)
  cat("\nCount for", var, ":\n")
  print(count_observations(wasps, !!sym(var)))
}

```

We see that the *block* variable has errors in Block "E" (one too many) and Block "D" (one too few)
The *position* variable has errors in Position 6 (one too many) and Position 2 (one too few)
...
The code is designed to identify errors in the wasps data by checking for inconsistencies in the occurrence of combinations of week, block, and position. Any inconsistencies found by the last line indicate a deviation.
...
table2 %>%
   ...
   pivot_wider(id_cols=c(week, block),
               names_from=position,
               values_from=n,
               values_fill=0)
```
...
3. The argument `values_fill=0` places a 0 in the combination of week, block, and position that is *not present* in the data set.
4. After `pivot_longer()`, the data set has *200* rows.
5. After `pivot_longer()`, the row with a value of *greater than 1* in the `n` column contains the week, block, and position that appears too often in the data set.
...
The observation where week=2, lure=Control, block=E, and position=6 is in the data set twice and should not be.  
...
- week from XXX to XXX (No change)
- block from E to D
- position from 6 to 2
- lure from XXX to XXX (No change)
